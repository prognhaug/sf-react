import React from "react";

interface TripletexProps {
  className?: string; // Define className as an optional string
}

const Tripletex: React.FC<TripletexProps> = ({ className }) => (
  <svg
    version="1.1"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    x="0px"
    y="0px"
    viewBox="0 0 126.4 42"
    // style={{ enableBackground: "new 0 0 126.4 42" }}
    xmlSpace="preserve"
    className={className}
  >
    <style type="text/css">
      {`.tripletex_st0{fill:#58B3CD;}
    .tripletex_st1{fill:#1AB96D;}
    .tripletex_st2{fill:#010A59;}`}
    </style>

    <path
      d="M65.5,28.4c-0.4-2.5-1.7-4.6-3.9-5.9c-0.9-0.5-1.9-0.8-2.9-0.9c-1-0.1-2,0-3,0.3c-4.2,1.2-6.3,5.7-5.2,9.7
		c0.3,1,0.7,1.9,1.4,2.7c0.6,0.8,1.4,1.5,2.3,2c0.9,0.5,1.9,0.8,2.9,0.9c1,0.1,2,0,3-0.3c1.4-0.4,2.7-1.2,3.6-2.4
		c1-1.3,1.6-2.8,1.8-4.4l-3.5,1c-0.3,1.3-1.2,2.4-2.8,2.9c-1.9,0.6-4.3-0.1-5-2.3L65.5,28.4z M53.6,29.1c-0.3-1.4,0.6-3.6,3.1-4.3
		c2.4-0.7,4.4,0.7,4.9,2L53.6,29.1z"
    ></path>
    <path
      d="M90.8,28.4c-0.4-2.5-1.7-4.6-3.9-5.9C86,22.1,85,21.8,84,21.7c-1-0.1-2,0-3,0.3c-4.3,1.2-6.4,5.7-5.2,9.7
		c0.3,1,0.8,1.9,1.4,2.7c0.6,0.8,1.4,1.5,2.3,2c0.9,0.5,1.9,0.8,2.9,0.9c1,0.1,2,0,3-0.3c1.4-0.4,2.7-1.2,3.7-2.4
		c1-1.3,1.7-2.8,1.8-4.4l-3.5,1c-0.3,1.3-1.2,2.4-2.8,2.9c-1.9,0.6-4.3-0.1-5.1-2.3L90.8,28.4z M78.8,29.1c-0.3-1.4,0.6-3.6,3.1-4.3
		c2.4-0.7,4.4,0.7,4.9,2L78.8,29.1z"
    ></path>
    <path d="M90.8,36.8H95l3.3-5l3.3,5h4.3l-5.4-7.7l4.9-6.9h-4.1l-2.8,4.5l-2.8-4.5h-4.2l4.9,6.9L90.8,36.8z"></path>
    <path
      d="M41.8,29.5c0,3.8-2.7,7.7-7.8,7.7c-1.5,0-3-0.4-4.3-1.2v6h-3.5V30.1c0-0.2,0-0.4,0-0.7v0c0-3.7,2.7-7.7,7.8-7.7
		C39.1,21.7,41.8,25.7,41.8,29.5z M38.3,29.4c0-2.6-1.9-4.5-4.3-4.5c-2.3,0-4.2,1.8-4.3,4.3h0V30c0.1,1.1,0.6,2.1,1.4,2.8
		c0.8,0.7,1.8,1.1,2.9,1.1C36.4,33.9,38.3,32,38.3,29.4"
    ></path>
    <path d="M47.4,17h-3.2v19.7h3.5v-3.1V17H47.4z"></path>
    <path
      d="M19.4,22.1h3.5v14.7h-3.5V22.1z M21.1,19.9c0.4,0,0.8-0.1,1.1-0.3c0.3-0.2,0.6-0.5,0.7-0.9c0.1-0.4,0.2-0.7,0.1-1.1
		c-0.1-0.4-0.3-0.7-0.5-1c-0.3-0.3-0.6-0.5-1-0.5c-0.4-0.1-0.8,0-1.1,0.1c-0.4,0.1-0.7,0.4-0.9,0.7s-0.3,0.7-0.3,1.1
		c0,0.5,0.2,1,0.6,1.4C20.1,19.7,20.6,19.9,21.1,19.9z"
    ></path>
    <path
      d="M11.5,29.3c0.1-2.5,2-4.3,4.4-4.3c0.3,0,0.6,0,0.8,0.1v-3.3c-0.3,0-0.6,0-0.8,0c-5.1,0-7.9,4-7.9,7.8v0c0,0.2,0,0.4,0,0.7
		v6.6h3.5L11.5,29.3z"
    ></path>
    <path d="M7.2,21.5H3.5V17H0v19.6h3.5V25h3.6V21.5z"></path>
    <path d="M74.9,21.5h-3.6V17h-3.5v19.6h3.5V25h3.6V21.5z"></path>
    <path
      className="tripletex_st0"
      d="M122.4,12.2v-0.7h4c-0.1-1-0.5-1.9-1.2-2.6c-0.7-0.7-1.6-1.1-2.5-1.2c-1-0.1-1.9,0.1-2.7,0.6
		c-0.8,0.5-1.4,1.3-1.7,2.2c-0.3,0.9-0.3,1.9,0,2.8s1,1.7,1.8,2.2c0.8,0.5,1.8,0.7,2.8,0.6c1-0.1,1.8-0.6,2.5-1.3
		c0.7-0.7,1.1-1.6,1.1-2.6L122.4,12.2z"
    ></path>
    <path
      className="tripletex_st1"
      d="M114.3,19.6h0.7v4c1-0.1,1.9-0.5,2.6-1.2c0.7-0.7,1.1-1.6,1.2-2.6c0.1-1-0.1-1.9-0.6-2.8
		c-0.5-0.8-1.3-1.4-2.2-1.8c-0.9-0.3-1.9-0.3-2.8,0c-0.9,0.3-1.7,1-2.2,1.8c-0.5,0.8-0.7,1.8-0.6,2.8c0.1,1,0.6,1.9,1.3,2.5
		c0.7,0.7,1.6,1.1,2.6,1.2L114.3,19.6z"
    ></path>
    <path
      className="tripletex_st2"
      d="M114.9,4.3l-0.5-0.5l2.8-2.8c-0.8-0.7-1.9-1.1-3-1s-2.1,0.5-2.9,1.4c-0.7,0.8-1.2,1.9-1.1,2.9
		c0,1.1,0.5,2.1,1.2,2.9c0.8,0.8,1.8,1.2,2.9,1.2c1.1,0,2.2-0.4,3-1.1c0.8-0.7,1.3-1.8,1.4-2.9c0.1-1.1-0.3-2.2-1-3L114.9,4.3z"
    ></path>
  </svg>
);

export default Tripletex;
